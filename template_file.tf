...
data "template_file" "user_data" {
  template = <<-EOF
    #!/bin/bash
    aws s3 cp s3://${data.aws_s3_object.this.bucket}/${data.aws_s3_object.this.key} /usr/local
    tar -xvf /usr/local/apache-tomcat-7.0.64-PL.tar -C /usr/local
    aws s3 cp s3://${data.aws_s3_object.war_file.bucket}/${data.aws_s3_object.war_file.key} /usr/local/apache-tomcat-7.0.64-PL/webapps/
